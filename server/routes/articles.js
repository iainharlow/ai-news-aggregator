...
router.get('/', async (req, res) => {
  try {
        const feedURL = req.query.feedUrl || 'http://feeds.bbci.co.uk/news/technology/rss.xml';
        ...
        if (feed.items.length > 0) {
           ...
           db.run(
            `INSERT INTO articles (title, link, full_text, published_date, feed_url) VALUES (?, ?, ?, ?, ?)
            ON CONFLICT(link) DO UPDATE SET full_text = ?, feed_url = ?`,
               [topArticle.title, topArticle.link, full_text, topArticle.pubDate, feedURL, full_text, feedURL],
            (err) => {
            if (err) {
